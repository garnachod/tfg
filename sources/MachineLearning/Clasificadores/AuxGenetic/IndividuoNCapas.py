...
class IndividuoNCapas(object):
	"""docstring for IndividuoNCapas"""
	def __init__(self, aClases, nColum):
		super(IndividuoNCapas, self).__init__()
		self.nRepPorClase = 3
		...
		self.repeticionesPorCapa = [3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3]
	...
	def duplica(self):
		indv = IndividuoNCapas(self.clases, self.nColumnas)
		for k in range(0, self.nCapas):
			nNeuronas = self.repeticionesPorCapa[k] * self.nClases
			for j in range(0, nNeuronas):
