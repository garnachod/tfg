...
class EscritorSiguiendoNeo4j(Escritor):
	"""docstring for EscritorSeguidoresNeo4j"""
	def __init__(self, searchID):
		super(EscritorSiguiendoNeo4j, self).__init__(searchID)
	...
	def write(self, nodos_crear, todos_nodos_ids, nodo_principal_id):
		...
		for i, identificador in enumerate(todos_nodos_ids):
			query = "MATCH (nf { id_twitter: "+str(nodo_principal_id)+" }),(np { id_twitter: "+str(identificador)+"}) MERGE (nf)-[r:FOLLOW]->(np) ON CREATE SET r.since = timestamp()" 
